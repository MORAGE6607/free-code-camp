...
echo "$($PSQL "SELECT DISTINCT teams.name FROM games JOIN teams ON (games.winner_id = teams.team_id OR games.opponent_id = teams.team_id) WHERE games.year = 2014 AND games.round = 'Eighth-Final'")"
